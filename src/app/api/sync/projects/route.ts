...
// Public endpoint - list all projects with published units (for website)
// NO auth required, NO private data exposed (developer/investor info hidden)
...
export async function GET() {
  ...
  const projects = db.prepare(`
    SELECT p.id, p.name, p.location, p.description, p.website_description, p.total_units, p.images, p.completion_date
    FROM projects p
    WHERE p.published = 1
  ...
  const formatted = projects.map(project => {
    ...
    return {
      ...
      description: (project.website_description as string) || (project.description as string) || '',
      ...
      sold_count: 0, // Don't expose sold count publicly
    };
  });
